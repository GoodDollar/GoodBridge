...
abstract contract BridgeCore {
    ...
    function submitBlocks(SignedBlock[] memory signedBlocks) public {
        for (uint256 i = 0; i < signedBlocks.length; i++) {
            ...
            for (uint256 j = 0; j < _block.signatures.length; j++) {
                (bytes32 r, bytes32 vs) = abi.decode(
                    _block.signatures[j],
                    (bytes32, bytes32)
                );
                signers[j] = ECDSA.recover(
            ...
            chainVerifiedBlocks[_block.chainId][
                blockHeader.number
            ] = rlpHeaderHash;
            ...
            if (
                isFuse &&
                _block.validators.length > 0 &&
                blockHeader.number >= validatorsCycleEnd
            ) {
    ...
    function _setValidators(address[] memory validators, uint256 cycleEnd)
        internal
    {
    ...
    function executeReceipts(uint256 chainId, BlockSignedTxs[] calldata blocks)
        public
        virtual
    {
        for (uint256 i = 0; i < blocks.length; i++) {
            BlockSignedTxs memory blockReceipts = blocks[i];
            bytes32 blockHash = chainVerifiedBlocks[chainId][
                blockReceipts.blockNumber
            ];
            require(
                keccak256(blockReceipts.blockHeaderRlp) == blockHash,
                "invalid block hash"
            );
            RLPReader.RLPItem[] memory ls = blockReceipts
                .blockHeaderRlp
                .toRlpItem()
                .toList();
            ...
            for (uint256 j = 0; j < blockReceipts.receiptProofs.length; j++) {
                bytes32 receiptKey = keccak256(
                    abi.encode(blockReceipts.receiptProofs[j])
                );
                require(
                    usedReceipts[receiptKey] == false,
                    "receipt already used"
                );
                usedReceipts[receiptKey] = true;
                require(
                    blockReceipts.receiptProofs[j].keyIndex == 0 &&
                        blockReceipts.receiptProofs[j].proofIndex == 0,
                    "not start index"
                );
                require(
                    blockReceipts.receiptProofs[j].expectedRoot == receiptRoot,
                    "receiptRoot mismatch"
                );
                require(
                    blockReceipts.receiptProofs[j].verifyTrieProof(),
                ...
                require(
                    _executeReceipt(
                        ...
                        blockReceipts.blockNumber,
                        RLPParser.toReceipt(
                            blockReceipts.receiptProofs[j].expectedValue
                        )
                    ),
                    "execute failed"
                );
    ...
    function parseRLPToHeader(bytes memory rlpHeader)
        public
        pure
    ...
    function verifyParentBlocks(
        ...
        require(
            childHash == keccak256(childRlpHeader),
            "invalid child rlpHeader"
        );
