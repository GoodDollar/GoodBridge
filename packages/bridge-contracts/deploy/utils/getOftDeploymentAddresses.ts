 ...
 *   deployments/<networkName>/GoodDollarMinterBurner.json
...
export function getOftDeploymentAddresses(networkName: string): OftDeploymentAddresses {
  ...
  const minterBurnerPath = path.join(deploymentsDir, networkName, 'GoodDollarMinterBurner.json');
  const oftAdapterPath = path.join(deploymentsDir, networkName, 'GoodDollarOFTAdapter.json');

  if (!fse.existsSync(minterBurnerPath)) {
    throw new Error(
      `Missing hardhat-deploy artifact for GoodDollarMinterBurner on network "${networkName}". ` +
  ...
  if (!minterBurnerDeployment.address) {
    throw new Error(`Invalid deployment file (no "address") for GoodDollarMinterBurner: ${minterBurnerPath}`);
  }
  ...
  return {
    GoodDollarMinterBurner: minterBurnerDeployment.address,
