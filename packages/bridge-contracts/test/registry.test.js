...
describe("BlockHeaderRegistry", async () => {
  ...
  beforeEach(async () => {
    ...
    BlockHeaderRegistry = await ethers.getContractFactory(
      "BlockHeaderRegistry"
    );
    blockHeaderRegistry = await BlockHeaderRegistry.deploy(
      voting.address,
      consensus.address
    );
  ...
  const addSigBlocks = async (signerToUse, rlpHeaderp) => {
    ...
    const payload = ethers.utils.keccak256(ethers.utils.solidityPack(
    ...
    const tx = await blockHeaderRegistry
      .connect(signerToUse)
      .addSignedBlocks(blockData(rlpHeaderp, bhash, r, vs));
  ...
  describe("Blockchains", () => {
    it("Should add a new blockchain", async () => {
      const tx = await voting.addBlockchain(
        blockHeaderRegistry.address,
        1337,
        "http://localhost:8545"
      );
      const rx = await tx.wait();
    ...
    it("Should not add a new blockchain from a random caller", async () => {
      ...
      await expect(
        blockHeaderRegistry
          .connect(signer)
          .addBlockchain(1337, "http://localhost:8545")
      ).to.be.revertedWith("onlyVoting");
    });
  });
  ...
  describe("Signed blocks", () => {
    ...
    it("Should return the block hash with most signatures", async () => {
      ...
      expect(block.blockHeader.blockHash).to.equal(payload2);
    });
    ...
    it("Should fail if the rlpHeaderHash is not the blockHash", async () => {
      ...
      await expect(
        blockHeaderRegistry
          .connect(signer)
          .addSignedBlocks([
            [rlpHeader.slice(0, -10), [r, vs], 1, bhash, 0, []],
          ])
      ).to.be.revertedWith("rlpHeaderHash");
    });

    it("Should fail if there are no signed blocks at some height", async () => {
      await expect(blockHeaderRegistry.getSignedBlock(1, 0)).to.be.revertedWith(
        "_blockHashes.length"
      );
    });
    ...
    it("Should skip already signed block at the same height", async () => {
      ...
      const tx = await blockHeaderRegistry
        .connect(signer)
        .addSignedBlocks([[rlpHeader, [r, vs], 1, bhash, 0, []]]);
      const rx = await tx.wait();
    ...
    it("Should add and get a EVM signed block", async () => {
      ...
      const block = await blockHeaderRegistry.getSignedBlock(1, header.Number);
      expect(block.blockHeader.blockHash).to.equal(ethers.utils.keccak256(rlpHeader));
    });
    ...
    it("Should add and get a fuse signed block", async () => {
      ...
      const tx = await blockHeaderRegistry
        .connect(signer)
        .addSignedBlocks([
          [rlpHeader, [r, vs], 0x7a, blockHash, cycleEnd, validators],
        ]);
      const rx = await tx.wait();
      expect(rx.status).to.equal(1);
      const bhash = await blockHeaderRegistry.blockHashes(
        0x7a,
        header.Number,
        0
      );
      expect(bhash).to.equal(payload);
      const block = await blockHeaderRegistry.getSignedBlock(
        0x7a,
        header.Number
      );
      expect(block.blockHeader.blockHash).to.equal(blockHash);
      const header2 = {
        ...header,
        ReceiptHash:
          "0x56e81f171bcc55a6ff8345e692c0f86e5b48e01b996cadc001622fb5e363b420",
      ...
      const tx2 = await blockHeaderRegistry
        .connect(signer)
        .addSignedBlocks([
          [rlpHeader2, [r2, vs2], 0x7a, blockHash2, cycleEnd, validators],
        ]);
      const rx2 = await tx2.wait();
      expect(rx2.status).to.equal(1);
      const block2 = await blockHeaderRegistry.getSignedBlock(
        0x7a,
        header2.Number
      );
      expect(block2.blockHeader.blockHash).to.equal(blockHash);
      ...
      const tx3 = await blockHeaderRegistry
        .connect(signers[2])
        .addSignedBlocks([
          [rlpHeader2, [r3, vs3], 0x7a, blockHash2, cycleEnd, validators],
        ]);
      const rx3 = await tx3.wait();
      const block3 = await blockHeaderRegistry.getSignedBlock(
        0x7a,
        header2.Number
      );
      expect(block3.blockHeader.blockHash).to.equal(blockHash2);
