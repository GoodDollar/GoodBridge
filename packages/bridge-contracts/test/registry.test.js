...
describe('BlockHeaderRegistry', () => {
  ...
  describe('Signed blocks', () => {
    ...
    it('Should fail if there are no signed blocks at some height', async () => {
      await expect(blockHeaderRegistry.getSignedBlock(1, 0)).to.be.revertedWith('_blockHashes.length');
    });
    ...
    it('Should skip already signed block at the same height', async () => {
      ...
      await expect(blockHeaderRegistry.connect(signer).addSignedBlocks([[rlpHeader, [r, vs], 1, bhash, 0, []]])).to.not
        .reverted;
    });
    ...
    it('Should not add a signed block from another signer', async () => {
      ...
      const { _vs: vs, r } = ethers.utils.splitSignature(await signer.signMessage(ethers.utils.arrayify(payload)));
      await expect(
        blockHeaderRegistry.connect(signers[2]).addSignedBlocks([[rlpHeader, [r, vs], 1, bhash, 0, []]]),
