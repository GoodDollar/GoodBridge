...
describe('Bridge', () => {
    ...
    before(async () => {
        ...
        bridgeA = await (await ethers.getContractFactory("TokenBridge")).deploy(validators,
            10000000,
            requiredValidators,
            0,
            token.address) as TokenBridge
        bridgeB = await (await ethers.getContractFactory("TokenBridge")).deploy(validators,
                10000000,
    ...
    describe("block proofs",() => {
        ...
        it("should submit signed block after 10 previous blocks", async () => {
            for(let i=0;i<10;i++)
                await signers[0].sendTransaction({value:0, to: signers[1].address})
            const block = await ethers.provider.send(
                'eth_getBlockByNumber',
                ["latest", false]
              );
           childHeader =  SignUtils.prepareBlock(block, 1337)
           let sig1 = await SignUtils.signBlock(childHeader.rlpHeader, 1337, signers[0], 0, [])
           const sig2 = await SignUtils.signBlock(childHeader.rlpHeader, 1337, signers[1], 0, [])
           const { signature, ...signedBlock } = {...sig1,...{signatures:[ethers.utils.defaultAbiCoder.encode(["bytes32","bytes32"],Object.values(sig1.signature)), ethers.utils.defaultAbiCoder.encode(["bytes32","bytes32"],Object.values(sig2.signature))]}}   
        ...
        it("should submit parent blocks", async () => {           
            const parents = await Promise.all(range(childHeader.block.number - 1, childHeader.block.number - 11).map(async idx => {
                const block = await ethers.provider.send(
                    'eth_getBlockByNumber',
                    ["0x"+idx.toString(16), false]
                  );
                return SignUtils.prepareBlock(block, 1337)
            }))
            const parentRlps = parents.map(_ => _.rlpHeader)
            await bridgeB.verifyParentBlocks(1337, childHeader.block.number,parentRlps, childHeader.rlpHeader);
        })
    })
    describe("transfer and receive", () => {
        ...
        it("should submit signed block to target bridge",async () => {
            const block = await ethers.provider.send(
                'eth_getBlockByNumber',
                ["0x"+tx.blockNumber.toString(16), true]
              );
        ...
        //    console.log({root: proof.receiptsRoot,expectedRoot, proofRoot: ethers.utils.keccak256(ethers.utils.RLP.encode(proof.orgProof[0]))})
            ...
            const mptProof = {expectedRoot, expectedValue: receiptRlp, proof:proof.receiptProof,key: SignUtils.index2key(proof.txIndex,proof.receiptProof.length), keyIndex:0, proofIndex:0}
            
            const res = bridgeB.executeReceipts(1337,[{receiptProofs:[mptProof],blockHeaderRlp: proof.headerRlp, blockNumber: tx.blockNumber}])
